...
function validatePanels(raw){
 ...
 const seen=new Set();const list=raw.panels.map(p=>{if(!p||!['side','shelf','back'].includes(p.type)||![p.x,p.y,p.z].every(v=>Number.isFinite(v)&&Number.isInteger(v*2))||![p.u??1,p.v??1].every(v=>v===.5||v===1)||Math.abs(p.x)>31||p.y<0||p.y>31||Math.abs(p.z)>31)throw Error('Invalid panel position.');if(!['mesh','plastic'].includes(p.material)||!/^#[0-9a-f]{6}$/i.test(p.color))throw Error('Invalid panel appearance.');const id=panelKey(p);if(seen.has(id))throw Error('Duplicate panel.');seen.add(id);return {type:p.type,x:p.x,y:p.y,z:p.z,material:p.material,color:p.color,...(p.u!==undefined?{u:p.u}:{}),...(p.v!==undefined?{v:p.v}:{})};});
 return {schema:'cube-studio',version:2,name:String(raw.name||'My booth display').slice(0,80),width:raw.width,height:raw.height,depth:raw.depth,panels:list,...(raw.table!==undefined?{table:tableSettings(raw.table)}:{})};
}

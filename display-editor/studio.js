...
remember=function(){};
const standDefaults={floor:'#547f70',side:'#c9ad83',wall:'#738cad'};
$('.threeControls').insertAdjacentHTML('afterend','<div class="threeControls" id="standColors"><strong>Part colours</strong>'+Object.entries({floor:'Floors',side:'Side panels',wall:'Walls'}).map(([key,label])=>`<label>${label} <input type="color" id="standColor-${key}" value="${PART_COLORS[key]}" style="width:48px;height:30px;padding:2px"></label>`).join('')+'</div>');
for(const key of Object.keys(standDefaults))$('#standColor-'+key).onchange=e=>{PART_COLORS[key]=e.target.value;studio.editSession=null;render();};
function restore(data){
  studio.restoring=true;
  try{
    Object.assign(PART_COLORS,standDefaults,data.partColors||{});for(const key of Object.keys(standDefaults))$('#standColor-'+key).value=PART_COLORS[key];
...
function validateProject(raw){
  ...
  const partColors={};for(const [key,fallback] of Object.entries({floor:'#547f70',side:'#c9ad83',wall:'#738cad'})){const color=raw.partColors?.[key]??fallback;if(typeof color!=='string'||!/^#[0-9a-f]{6}$/i.test(color))throw Error('Invalid part colour');partColors[key]=color;}
  return {partColors,schema:raw.schema,version:5,units:'mm',settings,trayGroups,products:safeProducts,fabrication:{kerf:finite(raw.fabrication?.kerf??0,0,3,'Kerf'),relief:finite(raw.fabrication?.relief??0,0,10,'Relief')},view:raw.view||{}};
}

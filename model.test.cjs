...
test('wheel zoom magnifies the view without moving the camera into the booth',()=>{const fs=require('node:fs'),vm=require('node:vm'),source=fs.readFileSync('app.js','utf8'),start=source.indexOf("canvas.addEventListener('wheel',"),end=source.indexOf('},{passive:false});',start)+19;let listener;const camera={zoom:1},orbit={distance:400};const ctx=vm.createContext({canvas:{addEventListener:(name,fn)=>listener=fn},camera,orbit,down:null,updateCamera:()=>{}});vm.runInContext(source.slice(start,end),ctx);listener({preventDefault(){},deltaY:-600});assert.ok(camera.zoom>1);assert.equal(orbit.distance,400);listener({preventDefault(){},deltaY:-100000});assert.equal(camera.zoom,8);});
